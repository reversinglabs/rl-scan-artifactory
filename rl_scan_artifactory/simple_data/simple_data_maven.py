...
class SimpleDataMaven(SimpleDataCommon):
    def __init__(
        self,
        file: FileInfo,
    ) -> None:
    ...
    def _make_simple_data_maven(
        self,
    ) -> Dict[str, Any]:
        k = "jar"
        if not self.file.uri.lower().endswith(k):
            self.simple_data = {}  # maven has no properties on the jar file by default, no need to check
    ...
    def make_simple_data(
        ...
        return self._make_simple_data_maven()
